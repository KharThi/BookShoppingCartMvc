...
namespace BookStoreApi.Repositories
{
    public class CartRepository : ICartRepository
    {
        ...
        public async Task<int> AddItem(int bookId, int qty, string email)
        {
            ...
            var cartItemCount = await GetCartItemCount(userId,email);
            return cartItemCount;
        }


        public async Task<int> RemoveItem(int bookId, string email)
        {
            string userId = await GetUserId(email);
            try
            {
        ...
        public async Task<ShoppingCart> GetUserCart(string email)
        {
        ...
        public async Task<int> GetCartItemCount(string email, string userId = "")
        {
            if (string.IsNullOrEmpty(userId))
            {
        ...
        public async Task<bool> DoCheckout(CheckoutModel model)
        {
            ...
            try
            {
                var userId = await GetUserId(model.Email);
                if (string.IsNullOrEmpty(userId))
                    throw new UnauthorizedAccessException("User is not logged-in");
                var cart = await GetCart(userId);
                if (cart is null)
		...
		public async Task<Order> DoCheckoutVNPay(CheckoutModel model)
		{
			using var transaction = _db.Database.BeginTransaction();
			try
			{
